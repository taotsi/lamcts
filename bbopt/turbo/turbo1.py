...
from .utils import from_unit_cube, latin_hypercube, to_unit_cube, rejected_sample
...
class Turbo1:
    ...
    def _create_candidates(self, X, fX, length, n_training_steps, hypers):
        ...
        X_cand = self.heuristic_sample(X, lb, ub, device, dtype)
        if len(X_cand) > self.n_cand:
            random_idx = np.random.randint(len(X_cand), size=self.n_cand)
            X_cand = X_cand[random_idx, :]
    ...
    def optimize(self):
        ...
        while self.n_evals < self.max_evals:
            ...
            if len(self.node.x_samples) == 0:
                X_init = rejected_sample(self.n_init, self.dim, self.region, self.lb, self.ub)
            else:
                unit_samples = to_unit_cube(self.node.x_samples, self.lb, self.ub)
                X_init = self.heuristic_sample(unit_samples, self.lb, self.ub, self.device, self.dtype)
                if len(X_init) > self.n_init:
                    random_idx = np.random.randint(len(X_init), size=self.n_init)
                    X_init = X_init[random_idx, :]
    ...
    def heuristic_sample(self, X, lb, ub, device, dtype):
        '''
        X is the points around whith neighbours are built. points in X are all normalized
        '''
        assert X.min() >= 0 and X.max() <= 1

        intersection_X = [x for x in X if self.is_unit_x_within_region(x, lb, ub)]

        samples = np.empty((0, self.dim))

        for inter_x in intersection_X:
            nlb = np.clip(inter_x - 1e-4, 0, 1)
            nub = np.clip(inter_x + 1e-4, 0, 1)
            neighbour_X = self.sobol_sample(inter_x, 5, nlb, nub, device, dtype)
            scaled_X = self.scale_neighbour(neighbour_X, inter_x, 0.90, lb, ub)
            temp_X = [x for x in scaled_X if self.is_unit_x_within_region(x, lb, ub)]
            if temp_X:
                samples = np.vstack((samples, temp_X))
    ...
    def scale_neighbour(self, X, x_center, p, lb, ub):
        cover_percentage = lambda X : len([x for x in X if self.is_unit_x_within_region(x, lb, ub)]) / len(X)

        result = deepcopy(X)
        if cover_percentage(result) < p:
            return result
        ...
        while True:
            d *= 2;
            temp_X = result + d
            if (temp_X > 1).any() or (temp_X < 0).any():
                break
            if cover_percentage(temp_X) < p:
                break
            result = temp_X

        return result

    def is_unit_x_within_region(self, ux, lb, ub):
        x = ux * (self.ub - self.lb) + self.lb
        return self.region.is_within_region(x) and (ux >= lb).all() and (ux <= ub).all()
